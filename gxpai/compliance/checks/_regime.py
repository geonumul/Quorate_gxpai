...
# 압력 관리 대상이 아닌 공간
#   근거: 2010 안내서 p.5(보관소는 청정도 관리 불요) · p.37(청정도 관리 불요 예시)
#   ※ 복도는 '중립'이 아니라 **기준면**이다. 봉쇄 검사에서 비교 대상이 되므로 따로 표시한다.
NEUTRAL_WORDS = (
    "보관", "창고", "기계실", "전기", "P.S", "PS", "계단", "엘리베이터",
    "사무", "휴게", "식당", "화장실", "샤프트", "덕트",
)
...
def infer_regime(name: str | None, grade: str | None = None) -> str:
    ...
    우선순위: 특수제제 > **복도/중립** > 분진공정 > 보호(기본)

    ★복도 판별이 분진공정보다 **먼저** 와야 한다.
      '정립실복도' 는 복도지 정립실이 아니다. 순서를 바꿨더니 봉쇄로 오판했다(시험이 잡음).
    """
    n = (name or "").replace(" ", "")

    if any(w in n for w in HAZARD_WORDS):
        return HAZARD
    if is_corridor(n) or any(w in n for w in NEUTRAL_WORDS):
        return NEUTRAL
