...
def door_barriers(doc, door_layers: list[str], walls: np.ndarray,
    ...
    for c, p0, p1 in iter_door_arcs(doc, door_layers):
        ...
        if h0 < _MIN_WALL_HITS and h1 < _MIN_WALL_HITS:
            ...
            continue
        if h0 == h1:
            # 둘 다 벽에 똑같이 닿았다 → 어느 쪽이 닫힌 위치인지 못 가른다.
            #
            # ★처음엔 여기서 그냥 건너뛰었다. 참고도면에서 호 85개 중 **3개**만 살아남았다.
            #   문을 3개만 찾으면 ADJ-001 이 "문이 없으니 위반 아님"으로 **진짜 위반을 숨긴다.**
            #   → 이럴 땐 **양쪽 다 그린다.** 하나는 벽 구멍을 막고, 다른 하나는 방 안으로
            #     뻗는 막다른 선이 된다(fill 이 돌아가므로 방을 쪼개지 않는다).
            #   막는 것이 목적이지 문짝을 그리는 것이 목적이 아니다.
            if h0 >= _MIN_WALL_HITS:
                out.append((c[0], c[1], p0[0], p0[1]))
                out.append((c[0], c[1], p1[0], p1[1]))
            continue
