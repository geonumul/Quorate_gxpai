...
- 기하 방향 확정. **의미도 강한 증거 확보(docs/기존데이터_분석.md)**: 차압도 내부 라벨로 화살표→방쌍 귀속(57/98),
  방향그래프 56간선 중 모순 1개(98% 일관), 계단/샤프트가 전부 도착점(SINK, 도착4·출발0) → 화살촉=저압 쪽(고압→저압).
  상태 "미확정"→"거의 확정, 발주처 1줄 확인만". 확인 전까지 PRES 규칙은 계속 게이트(정반대 오판 방지).
- 부수 발견: TA 값(90~300, 이산군집)=풍량(CMH)계열, 차압(Pa) 아님 → PRES-002 는 TA로 불가. 차압도-평면도 좌표계 상이.
