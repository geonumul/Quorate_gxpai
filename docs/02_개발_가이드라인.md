...
└── docs/
    ├── 31_신규시설_온보딩.md # 신규 zip 수령 시 절차서
    ├── 30_규칙_작성법.md
    └── 92_포트폴리오.md               # 포트폴리오 산출물 트래킹
```
...
각 태스크: **목표 / 방법 / 함정, 폴백 / Acceptance(실행 확인)**. `docs/01_진행상황.md`에 체크 상태 유지.
...
- [ ] **T1.3.1** `docs/31_신규시설_온보딩.md` 작성: zip 수령→facility add→inventory→
...
- Web CAD/전자서명/대시보드는 계획서상 타 팀 스코프. 우리는 API Contract(JSON 스키마)를
  `docs/34_API_계약.md`로 선제 문서화 → 협업 시 주도권
...
## D.2 작업 세션 프로토콜
1. docs/02_개발_가이드라인.md → 01_진행상황.md → 현재 태스크 확인
2. 태스크 1개 → 구현 → **Acceptance 실행 로그를 01_진행상황.md에 붙임** → 커밋 `[S2.1] ...`
3. 15분/3회 룰: 막히면 태스크에 명시된 폴백 실행 or question 테이블 기록 후 다음 태스크
4. DB 스키마 변경 = 마이그레이션 파일. Neo4j 적재 = 멱등. 대형 DXF = 캐시 확인 먼저
5. 세션 종료: 01_진행상황.md에 "다음 첫 태스크" 1줄
...
# PART E. 포트폴리오 마감 기준 (docs/92_포트폴리오.md로 관리)
...
6. **연구 접점 메모**: 방-인접-압력 그래프는 본인 전공(그래프 학습, 건설 안전 XAI)과 직결 —
   "GMP 시설 그래프에서 위반 패턴 학습" 같은 후속 연구 아이디어를 92_포트폴리오.md에 축적
