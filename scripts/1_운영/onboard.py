...
if len(sys.argv) < 2:
    sys.exit("사용법: python scripts/1_운영/onboard.py <도면.dxf>")
p = Path(sys.argv[1])
...
if "--yaml" in sys.argv:
    ...
    if a["시트"]:
        ...
        for nm, cnt, why in a["시트"]:
            w(f"#   {nm!r}: {why}\n")
        w("#   → scripts/2_시트전처리/cut_ref_sheet_aligned.py 로 기준 시트 좌표계로 옮겨 잘라 온다.\n")
    print("\n" + "="*70 + "\n" + y.getvalue())
...
if "--try" in sys.argv:
    ...
    if best:
        ...
        print("     → 실패한 방 주변 6m 안의 레이어를 세어 볼 것(scripts/5_도면조사/diag_room_walls.py).")
